...
export const COPUZZLE_RULES_PRESET_KEY = 'copuzzle';

function asArray(value) {
  return Array.isArray(value) ? value : [];
}

export function getCopuzzleRulesPreset(localeInput = 'fr') {
  const locale = normalizeLocale(localeInput);
  const dictionary = getDictionary(locale);
  const node = dictionary?.challengeRules?.[COPUZZLE_RULES_PRESET_KEY] || {};

  return {
    key: COPUZZLE_RULES_PRESET_KEY,
...
export default getCopuzzleRulesPreset;