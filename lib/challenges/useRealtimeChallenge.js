...
export function refreshChallengeStateBeforeStart(emitEvent) {
  if (typeof emitEvent !== 'function') return;
  // Ask server for the freshest participant/status snapshot before start validation.
  emitEvent('participants.request_state', {});
  emitEvent('quiz.request_state', {});
  emitEvent('timer.request_state', {});
  emitEvent('laby.request_state', {});
  emitEvent('mission.request_state', {});
  emitEvent('vom.request_state', {});
  emitEvent('pixel.request_state', {});
  emitEvent('puzzle.request_state', {});
  emitEvent('phrase.request_state', {});
  emitEvent('lab.request_state', {});
}
  ...
  useEffect(() => {
    ...
    const onEvent = (packet = {}) => {
      ...
      if (type === 'lab.state') {
        setState((prev) => ({
          ...(prev || {}),
          labInnovation: payload?.labInnovation || payload?.lab_innovation || (prev?.labInnovation || null),
          config: payload?.config || (prev?.config || null),
        }));
      }
      ...
      if (type === 'participants.update') {
        ...
        const participantIds = participants
          .map((item) => String(item?.participant_id || item?.id || item?.participantId || ''))
          .filter(Boolean);
        ...
        setState((prev) => ({
          ...
          vom: prev?.vom
            ? {
                ...(prev.vom || {}),
                participants_meta: participants.length > 0
                  ? participants
                  : (prev.vom?.participants_meta || []),
                participants_order: participantIds.length > 0
                  ? participantIds
                  : (prev.vom?.participants_order || []),
              }
            : prev?.vom || null,
          quiz: prev?.quiz
            ? {
