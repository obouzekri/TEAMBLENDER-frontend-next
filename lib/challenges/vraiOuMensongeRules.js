...
export const VRAI_OU_MENSONGE_RULES_PRESET_KEY = 'pariSurMoi';

function asArray(value) {
  return Array.isArray(value) ? value : [];
}

export function getVraiOuMensongeRulesPreset(localeInput = 'fr') {
  const locale = normalizeLocale(localeInput);
  const dictionary = getDictionary(locale);
  const node = dictionary?.challengeRules?.[VRAI_OU_MENSONGE_RULES_PRESET_KEY] || {};

  return {
    key: VRAI_OU_MENSONGE_RULES_PRESET_KEY,
...
export default getVraiOuMensongeRulesPreset;
