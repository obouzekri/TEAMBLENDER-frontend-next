...
import {
  ...
  getStoredCurrentUser as readStoredCurrentUser,
  setStoredAuthToken,
  setStoredCurrentUser,
} from './auth-storage.js';
import { withCsrfHeaders } from './csrf.js';

const TARGET_SESSION_ID_STORAGE_KEY = 'targetSessionId';
...
export async function ensureCsrfToken() {
  ...
  if (!csrfWarmupPromise) {
    csrfWarmupPromise = fetch(getApiUrl('/auth/csrf-token'), {
      method: 'GET',
      credentials: 'include',
      cache: 'no-store',
    })
      .catch(() => {
        // Keep flow resilient; CSRF header may still be available from existing cookie.
      })
      .finally(() => {
...
export function setStoredAuthSession({ token, user, targetSessionId = '' } = {}) {
  if (typeof window === 'undefined') return;

  const isParticipant = String(user?.role || '').trim().toLowerCase() === 'participant';
  setStoredAuthToken(token, undefined, { persistLocal: !isParticipant });
  ...
  if (normalizedSessionId) {
    sessionStorage.setItem(TARGET_SESSION_ID_STORAGE_KEY, normalizedSessionId);
  } else {
    sessionStorage.removeItem(TARGET_SESSION_ID_STORAGE_KEY);
  }
}
...
export function getAuthHeaders(extraHeaders = {}) {
  return {
    ...
    ...buildAuthHeaders(withCsrfHeaders(extraHeaders)),
  };
}

export function shouldStoreParticipantTargetSession(role, requestedSessionId) {
  return String(role || '').trim().toLowerCase() === 'participant'
    ? String(requestedSessionId || '').trim()
    : '';
}

export function sanitizeJwtFromStorage(localToken, sessionToken) {
  return String(sessionToken || '').trim() || String(localToken || '').trim() || '';
}
...
export async function loginWithFallback(identifier, password, options = {}) {
  ...
  const participantCredentials = isEmailIdentifier
    ? { email: normalizedIdentifier.toLowerCase(), password }
  ...
  const participantRes = await fetch(getApiUrl('/auth/login-participant'), {
    ...
    headers: withCsrfHeaders({ 'Content-Type': 'application/json' }),
    credentials: 'include',
    body: JSON.stringify(participantCredentials),
  });
...
export async function joinParticipantInstant({ sessionCode, inviteToken, firstName, lastName, email, nickname, qrCode }) {
  await ensureCsrfToken();

  const payload = {
    first_name: String(firstName || nickname || '').trim(),
  ...
  if (normalizedCode) {
    payload.sessionCode = normalizedCode;
  }
  if (normalizedInviteToken) {
    payload.inviteToken = normalizedInviteToken;
  }
  if (normalizedQr) {
    payload.qrCode = normalizedQr;
  }

  const res = await fetch(getApiUrl('/auth/join-participant'), {
    method: 'POST',
    headers: withCsrfHeaders({ 'Content-Type': 'application/json' }),
    credentials: 'include',
...
export async function joinParticipantAuthenticated(sessionCode) {
  await ensureCsrfToken();
  const res = await fetch(getApiUrl('/auth/join-participant-authenticated'), {
    method: 'POST',
    headers: withCsrfHeaders({ 'Content-Type': 'application/json' }),
    credentials: 'include',
    body: JSON.stringify({ sessionCode: String(sessionCode || '').trim().toUpperCase() }),
  });
  const text = await res.text();
  let data = {};
