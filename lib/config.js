...
const API_BASE_FROM_ALIAS = resolveApiBaseWithPublicBackend(process.env.NEXT_PUBLIC_API_URL);
export const API_BASE = resolveBrowserApiFallback(RESOLVED_API_BASE);
export const API_BASE_ALIAS = resolveBrowserApiFallback(API_BASE_FROM_ALIAS);
...
export function normalizeBackendAssetUrl(rawValue) {
  ...
  if (/^https?:\/\//i.test(candidate)) {
    try {
      ...
      const normalizedAbsolute = `${parsed.protocol}//${parsed.host}${parsed.pathname}${parsed.search}${parsed.hash}`;
      ...
      const isBrowserSameHostUpload = browserOrigin
        && parsed.pathname.startsWith('/uploads/')
        && normalizedAbsolute.startsWith(browserOrigin);
      if (isBrowserSameHostUpload) {
...
function extractUploadsPath(rawValue) {
  const candidate = String(rawValue || '').trim();
  if (!candidate) return '';
  ...
  if (/^https?:\/\//i.test(candidate)) {
    try {
      ...
      if (parsed.pathname.startsWith('/api/uploads/')) {
        return `${parsed.pathname.slice(4)}${parsed.search}${parsed.hash}`;
      }
    } catch {
      return '';
    }
...
export function buildBackendAssetCandidates(rawValue) {
  const normalized = normalizeBackendAssetUrl(rawValue);
  const uploadPath = extractUploadsPath(rawValue) || extractUploadsPath(normalized);
  ...
  if (uploadPath) {
    pushCandidate(`/api${uploadPath}`);
    pushCandidate(uploadPath);

    if (PUBLIC_BACKEND_ASSET_ORIGIN && /^https?:\/\//i.test(PUBLIC_BACKEND_ASSET_ORIGIN)) {
      pushCandidate(`${PUBLIC_BACKEND_ASSET_ORIGIN}${uploadPath}`);
    }

    if (BACKEND_ORIGIN && /^https?:\/\//i.test(BACKEND_ORIGIN)) {
      pushCandidate(`${BACKEND_ORIGIN}${uploadPath}`);
    }

    pushCandidate(`${PRODUCTION_BACKEND_ORIGIN}${uploadPath}`);
  }
...
export function getApiUrl(path) {
  ...
  const base = API_BASE === '/api' && API_BASE_ALIAS !== '/api' ? API_BASE_ALIAS : API_BASE;
