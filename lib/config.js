...
function resolveApiBaseWithPublicBackend(rawApiBase) {
  ...
  if (!preferredBackendOrigin || !/^https?:\/\//i.test(preferredBackendOrigin)) {
    ...
    return isProductionRuntime ? `${PRODUCTION_BACKEND_ORIGIN}/api` : normalizedApiBase;
  }
...
function resolveBrowserApiFallback(apiBase) {
  ...
  if (apiBase === '/api') {
    // Temporary hardening: production domain can bypass /api proxy misroutes.
    return `${PRODUCTION_BACKEND_ORIGIN}/api`;
  }

  // Also harden absolute frontend-domain API bases (e.g. https://teamblender.io/api).
  if (/^https?:\/\//i.test(apiBase)) {
    try {
      ...
      if (isFrontendApiHost && parsed.pathname.startsWith('/api')) {
        return `${PRODUCTION_BACKEND_ORIGIN}${parsed.pathname}`;
      }
    } catch {
      return apiBase;
    }
