...
const FALLBACK_POLL_START_DELAY_MS = 500;
...
export function useSessionState(sessionId) {
  ...
  const hasTerminalAccessErrorRef = useRef(false);
  ...
  const fetchSessionState = useCallback(async () => {
    if (!sessionId || hasTerminalAccessErrorRef.current) return;

    try {
      const state = await fetchAPI(`/sessions/${sessionId}/state`, {
        method: 'GET'
      });
      hasTerminalAccessErrorRef.current = false;
      ...
      if (status === 400 || status === 403 || status === 404) {
        hasTerminalAccessErrorRef.current = true;
        stopFallbackPolling();
      }
