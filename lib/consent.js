...
const CONSENT_REOPEN_EVENT_NAME = 'teamblender-consent-reopen';
...
// Retrait du consentement : reaffiche le bandeau sans effacer la decision en cours.
export function openConsentPreferences() {
  const activeWindow = getWindow();
  if (!activeWindow) return;

  activeWindow.dispatchEvent(new Event(CONSENT_REOPEN_EVENT_NAME));
}

export function subscribeConsentReopen(callback) {
  const activeWindow = getWindow();
  if (!activeWindow || typeof callback !== 'function') {
    return () => {};
  }

  const handleReopenEvent = () => callback();
  activeWindow.addEventListener(CONSENT_REOPEN_EVENT_NAME, handleReopenEvent);

  return () => {
    activeWindow.removeEventListener(CONSENT_REOPEN_EVENT_NAME, handleReopenEvent);
  };
}
